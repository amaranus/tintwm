...
void set_opacity(struct client *c) {
  ...
  for (struct client *tmp = master; tmp; tmp = tmp->next) {
    if (tmp != c) {
      xcb_change_property(dpy, XCB_PROP_MODE_REPLACE, tmp->window, opacity_atom,
                          XCB_ATOM_CARDINAL, 32, 1, &inactive_opacity);
    }
  }

  // Aktif pencereyi tamamen opak yap
  xcb_change_property(dpy, XCB_PROP_MODE_REPLACE, c->window, opacity_atom,
                      XCB_ATOM_CARDINAL, 32, 1, &active_opacity);
}
...
static void activate_wm(void) {
  const uint32_t event_mask = XCB_EVENT_MASK_SUBSTRUCTURE_REDIRECT |
                              ...
                              XCB_EVENT_MASK_POINTER_MOTION; // Yeni eklenen
  xcb_void_cookie_t attr = xcb_change_window_attributes_checked(
...
static void setup(void) {
  ...
  if (!xcb_ewmh_init_atoms_replies(ewmh, cookie, NULL))
    DIE("Failed to initialize EWMH atoms\n");
...
static void motion_notify(xcb_motion_notify_event_t *e) {
  struct client *c = find_client(e->event);
  if (c && c != focus) {
    focus_client(c);
  }
...
static void map_request(xcb_map_request_event_t *e) {
  static xcb_atom_t net_wm_window_type = 0, net_wm_window_type_dialog = 0,
                    wm_transient_for = 0;
  ...
  if (trans_reply && xcb_get_property_value_length(trans_reply)) {
    ...
    // Dialog pencerelerini focus'layalım
...
static void run(void) {
  while (running) {
    ...
    case XCB_MOTION_NOTIFY:
      motion_notify((xcb_motion_notify_event_t *)ev);
      break;
    }
    free(ev);
    if (xcb_connection_has_error(dpy))
